 ...
 */
@RestController
@RequestMapping(ConstantesRest.URL_PUBLIC + "/funcionAgente")
public class FuncionAgenteController {

    @Inject
    private FuncionAgenteService funcionAgenteService;

    @RequestMapping(value = "/{id}", method = RequestMethod.GET)
    public FuncionAgente get(@PathVariable("id") Long id) {
        return funcionAgenteService.get(id);
    }

    @JsonView(JsonViews.List.class)
    @RequestMapping(value = "/findAll", method = RequestMethod.GET)
    public ResponseEntity<Page<FuncionAgente>> findAll(PageableFilter filter) {
        if (filter != null) {
            return new ResponseEntity<>(funcionAgenteService.filter(filter), HttpStatus.OK);
        }
