 ...
 **/
@RestController
@RequestMapping(ConstantesRest.URL_PUBLIC + "/subidaLibro")
public class SubidaLibroController {

    @Inject
    private SubidaLibroService subidaLibroService;

    @RequestMapping(value = "/{id}", method = RequestMethod.GET)
    public SubidaLibro get(@PathVariable("id") Long id) {
        return subidaLibroService.get(id);
    }

    @JsonView(JsonViews.List.class)
    @RequestMapping(value = "/findAll", method = RequestMethod.GET)
    public ResponseEntity<Page<SubidaLibro>> findAll(PageableFilter filter) {
        if (filter != null) {
            return new ResponseEntity<>(subidaLibroService.filter(filter), HttpStatus.OK);
        }
