...
/**
 * Created by hlorenzo on 02/08/2017.
 **/
@RestController
@RequestMapping(ConstantesRest.URL_PUBLIC + "/cita-libro")
public class CitaLibroController {
    ...
    public CitaLibro get(@PathVariable("id") Long id) {
        return citaLibroService.get(id);
    }

    @JsonView(JsonViews.List.class)
    @RequestMapping(value = "/findAll", method = RequestMethod.GET)
    public ResponseEntity<Page<CitaLibro>> findAll(PageableFilter filter) {
        if (filter != null) {
            return new ResponseEntity<>(citaLibroService.filter(filter), HttpStatus.OK);
        }
