 ...
 */
@RestController
@RequestMapping(ConstantesRest.URL_PUBLIC + "/tipologia")
public class TipologiaController {
    ...
    public Tipologia get(@PathVariable("id") Long id) {
        return tipologiaService.get(id);
    }

    @JsonView(JsonViews.List.class)
    @RequestMapping(value = "/findAll", method = RequestMethod.GET)
    public ResponseEntity<Page<Tipologia>> findAll(PageableFilter filter) {
        if (filter != null) {
            return new ResponseEntity<>(tipologiaService.filter(filter), HttpStatus.OK);
        }
